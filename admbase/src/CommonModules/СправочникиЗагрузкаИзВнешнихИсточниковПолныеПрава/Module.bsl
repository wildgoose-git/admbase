...
Процедура ЗагрузкаСправочникаСотрудникиСТаблицейОшибок(ТаблицаЗначений,ТаблицаОшибок) Экспорт 
	...
	Запрос.УстановитьПараметр("ТаблицаОшибок",		ТаблицаОшибок.Выгрузить());
...
#КонецОбласти
	...
	Выборка = Результат[0].Выбрать(); //запись загружаемого ФИО в подчиненный справочник 
	...
	Выборка = Результат[1].Выбрать(); //запись новых справочников
	Пока Выборка.Следующий() Цикл
		НачатьТранзакцию();
		Попытка
	...
	Выборка = Результат[2].Выбрать(); //запись исключаемых ФИО в справочник ИсключаемыеИзЗагрузкиДанные
	Пока Выборка.Следующий() Цикл
