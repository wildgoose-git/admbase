...
Процедура ПолучитьИзменненыеСостоянияСотрудниковERP(ДатаСведений,ДанныеФормыКоллекция) Экспорт 
	...
	СправочникиЗагрузкаИзВнешнихИсточниковПолныеПрава.ЗагрузкаСправочникаПоНаменованию(ТаблицаСостояний,"Подразделения","ПодразделениеСтрока");
...
Функция СформироватьИсходныеДанныеЗаявкиDMC(ДатаСведений,МассивОшибок,МассивСтруктур) Экспорт

	//Данные из другой базы получае только один раз за один вызов
	Если МассивСтруктур.Количество() = 0 Тогда

		ТипПодключения = ПолучитьТипПодключения("DMC");
	...
	Иначе
		//формирование Исходной	 таблицы из массива
		ИсходнаяТаблица = КонсолидацияТаблицыЗначений.ИнициализироватьТаблицуЗаявок();
		Для Каждого ЭлементМассива Из МассивСтруктур Цикл
			ЗаполнитьЗначенияСвойств(ИсходнаяТаблица.Добавть(),ЭлементМассива);
		КонецЦикла;
	КонецЕсли;
...
Функция СформироватьИсходныеДанныеОтклонений(ДатаСведений,МассивОшибок,МассивСтруктур) Экспорт

	Если МассивСтруктур.Количество() = 0 Тогда
		ТипПодключения = ПолучитьТипПодключения("ERP");//Справочники.УчетныеДанныеПодключений.ERP.ТипПодключения;

		Если ТипПодключения = Перечисления.ТипыПодключения.ПодключениеCOM Тогда
			ИсходнаяТаблица = ТаблицаОтклонений_COMСоединение(ДатаСведений);
		ИначеЕсли ТипПодключения = Перечисления.ТипыПодключения.WebСервис Тогда
			ИсходнаяТаблица = ТаблицаОтклонений_WEBСервис(ДатаСведений);
		Иначе
			ИсходнаяТаблица = ТаблицаОтклонений_ЗапросHTTP(ДатаСведений);
		КонецЕсли;
		
		//преобразование в массив структур
		Для Каждого СтрокаТаблицы Из ИсходнаяТаблица Цикл
			Структура = КонсолидацияКлиентСервер.ИнициализироватьСтруктуруТаблицыОтклонений();
			МассивСтруктур.Добавить(Структура);
		КонецЦикла;
	Иначе
		//формирование Исходной	 таблицы из массива
		ИсходнаяТаблица = КонсолидацияТаблицыЗначений.ИнициализироватьТаблицуОтклонений();
		Для Каждого ЭлементМассива Из МассивСтруктур Цикл
			ЗаполнитьЗначенияСвойств(ИсходнаяТаблица.Добавть(),ЭлементМассива);
		КонецЦикла;
	КонецЕсли;
	...
	ЗагрузкаСправочникаСотрудникиСОбработкойОшибок(ИсходнаяТаблица,МассивОшибок);
	СправочникиЗагрузкаИзВнешнихИсточниковПолныеПрава.ЗагрузкаСправочникаПоНаменованию(ИсходнаяТаблица,"ТипыДокументов","ТипДокументаСтрока");
	
	ТаблицаОтклонений = СформироватьТаблицуОтклонений(ИсходнаяТаблица);
...
Функция СформироватьИсходныеДанныеСостоянияСотрудников(ДатаСведений,МассивОшибок,МассивСтруктур) Экспорт 
	
	Если МассивСтруктур.Количество() = 0 Тогда
		ТипПодключения = ПолучитьТипПодключения("ERP");//Справочники.УчетныеДанныеПодключений.ERP.ТипПодключения;

		Если ТипПодключения = Перечисления.ТипыПодключения.ПодключениеCOM Тогда
			ИсходнаяТаблица = ТаблицаСостояний_COMСоединение(ДатаСведений);
		ИначеЕсли
		ТипПодключения = Перечисления.ТипыПодключения.WebСервис Тогда
			ИсходнаяТаблица = ТаблицаСостояний_WEBСервис(ДатаСведений);
		Иначе
			ИсходнаяТаблица = ТаблицаСостояний_ЗапросHTTP(ДатаСведений);
		КонецЕсли;
	...
	СправочникиЗагрузкаИзВнешнихИсточниковПолныеПрава.ЗагрузкаСправочникаПоНаменованию(ИсходнаяТаблица,"Подразделения","ПодразделениеСтрока");
	СправочникиЗагрузкаИзВнешнихИсточниковПолныеПрава.ЗагрузкаСправочникаПоНаменованию(ИсходнаяТаблица,"Должности","ДолжностьСтрока");
	
	ТаблицаСостояний = СформироватьТаблицуЗагрузкиСостоянийСотрудников(ИсходнаяТаблица,КонецДня(ДатаСведений));
	
	Массив = Новый Массив;
	Для Каждого СтрокаТаблицы Из ТаблицаСостояний Цикл
		Структура = КонсолидацияКлиентСервер.ИнициализироватьСтруктуруТаблицыСостояний();
		ЗаполнитьЗначенияСвойств(Структура,СтрокаТаблицы);
		Массив.Добавить(Структура);
	КонецЦикла;
...
#Область ТекстЗапроса
	...
	|	ТаблицаЗначений.ПодразделениеСтрока КАК ПодразделениеСтрока
	|ПОМЕСТИТЬ ТаблицаЗначений
	|ИЗ
	|	&ТаблицаЗначений КАК ТаблицаЗначений
	|;
	...
	|		ПО ТаблицаЗначений.ПодразделениеСтрока = СправочникПодразделения.Наименование
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗРЕШЕННЫЕ
...
Функция ТаблицаСостояний_ЗапросHTTP(ДатаСведений)
	...
	Если ОтветHTTP.КодСостояния = 200 Тогда
		...
		Для Каждого Выборка Из Данные Цикл
			...
			НоваяСтрока.ПодразделениеСтрока = Выборка.ПодразделениеСтрока;
...
Функция ТаблицаСостояний_WEBСервис(ДатаСведений)  
	...
	Для Каждого Выборка Из Данные.RecordslList Цикл
		...
		НоваяСтрока.ПодразделениеСтрока = Выборка.Subdivision;
...
Функция ТекстЗапросаСведенияОСотрудниках()  
	...
	|	ЕСТЬNULL(ДанныеДляПодбора.ПодразделениеНаименование,"""") КАК ПодразделениеСтрока,
...
#Область ЗагрузкаСправочникаСотрудники
// Формирует массив ошибок, создает элементы справочника Сотрудники 
// 
// Параметры:
//  ИсходнаяТаблица - ТаблицаЗначений - Исходная таблица:
// * ФИОСотрудника - Строка 
//  МассивОшибок - Структура - МассивСтруктур см. КонсолидацияКлиентСервер.ИнициализироватьСтруктуруТаблицыОшибок
...
Функция ПолучитьТипПодключения(ИмяПредопределенногоРеквизита)
	Запрос = Новый Запрос("ВЫБРАТЬ
						  |	УчетныеДанныеПодключений.ТипПодключения
						  |ИЗ
						  |	Справочник.УчетныеДанныеПодключений КАК УчетныеДанныеПодключений
						  |ГДЕ
						  |	УчетныеДанныеПодключений.Ссылка = Значение(Справочник.УчетныеДанныеПодключений.%1)");
	Запрос.Текст = СтрШаблон(Запрос.Текст,ИмяПредопределенногоРеквизита);
	Выборка = Запрос.Выполнить().Выбрать();
	Выборка.Следующий();
	Возврат Выборка.ТипПодключения;
КонецФункции // ПолучитьТипПодключения()
